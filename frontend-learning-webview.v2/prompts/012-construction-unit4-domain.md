# 프롬프트 012: Construction - Unit 4 도메인 설계

```bash
2단계: Unit 4 구축 (Construction)
단계 2.1: DDD를 사용한 도메인 모델 설계

당신의 역할: 당신은 전문 소프트웨어 아키텍트로서, Unit 4에 대해 DDD 경량화 방식을 사용하여
...
당신의 과제: aidlc-docs/inception/units/ 폴더를 참조하세요. Unit 4 md 파일에는 개선 목표와 범위가
정의되어 있습니다.

DDD 경량화 방식으로 도메인 모델을 설계하세요.

aidlc-docs/construction/unit-04-orchestration/domain_design.md 파일에 설계 세부사항을 작성하세요.
