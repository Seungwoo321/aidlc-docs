...
이 문서는 E-Torch 프로젝트의 7개 **Feature Module** 간 통합 계약을 정의합니다.
각 Feature Module은 독립적으로 개발 가능하며, 명확한 인터페이스를 통해 통신합니다.
...
│                      Next.js API Routes                      │
│              (HTTP → TypeScript 함수 호출 래핑)                │
└─────────────────────┬───────────────────────────────────────┘
                      │
                      ▼
┌─────────────────────────────────────────────────────────────┐
│              Feature Module Layer (TypeScript)               │
│                                                              │
│  이 문서에 정의된 Contract는 TypeScript 함수 계약입니다         │
│  - TypeScript Interface로 타입 정의                           │
│  - 구현 단계에서 Zod Schema로 런타임 검증 추가                 │
│  - BFF API Routes가 이 함수들을 HTTP 엔드포인트로 노출          │
└─────────────────────────────────────────────────────────────┘
```

### 계약의 성격

- **내부 계약**: TypeScript 함수 호출 기반 (Feature Module 간)
- **외부 노출**: BFF API Routes가 HTTP 엔드포인트로 변환
- **타입 안정성**: TypeScript Interface로 컴파일 타임 검증
- **런타임 검증**: Zod Schema로 요청/응답 데이터 검증 (구현 단계)

## Feature Module 간 의존성 다이어그램

```
┌─────────────────────────────────────────────────────────────┐
│                    Admin Applications                        │
│                  (독립 Feature Modules / MFA)                 │
│                                                              │
│  ┌────────────────────┐   ┌────────────────────┐           │
│  │  Admin Console     │   │ Admin Moderation   │           │
│  │  (시스템 운영)        │   │ (컨텐츠 안전)        │           │
│  └────────────────────┘   └────────────────────┘           │
└─────────────────────────────────────────────────────────────┘
...
│         ▲                     ▲                              │
│         └─────────┬───────────┘                              │
│         All Feature Modules depend on these                  │
└─────────────────────────────────────────────────────────────┘
```
...
```
GET    /api/dashboards              - 대시보드 목록 조회 (내 대시보드)
GET    /api/dashboards/public       - 공개 대시보드 목록
GET    /api/dashboards/bookmarked   - 즐겨찾기한 대시보드 목록
GET    /api/dashboards/:id          - 대시보드 상세 조회
POST   /api/dashboards              - 대시보드 생성 (빈 대시보드, 템플릿, 복제)
PATCH  /api/dashboards/:id          - 대시보드 정보 수정
DELETE /api/dashboards/:id          - 대시보드 삭제
PATCH  /api/dashboards/:id/layout   - 레이아웃 수정
POST   /api/dashboards/:id/widgets  - 대시보드에 위젯 추가
DELETE /api/dashboards/:id/widgets/:widgetId - 대시보드에서 위젯 제거
GET    /api/dashboards/:id/versions - 버전 이력 조회 (Pro)
POST   /api/dashboards/:id/versions/:versionId/restore - 버전 복원 (Pro)
POST   /api/dashboards/:id/bookmark - 즐겨찾기 추가
DELETE /api/dashboards/:id/bookmark - 즐겨찾기 제거
POST   /api/dashboards/:id/share    - 공유 링크 생성
POST   /api/dashboards/:id/report   - 대시보드 신고 (US2.14)
POST   /api/consent                 - 동의 로그 기록 (US2.12, US2.13)
GET    /api/disclaimer/seen         - 면책 조항 확인 여부 (US2.13)
```

## 2. Widget Library Feature Module
...
```
GET    /api/widgets           - 내 위젯 목록 조회
GET    /api/widgets/public    - 공개 위젯 목록 조회
GET    /api/widgets/:id       - 위젯 상세 조회
POST   /api/widgets           - 위젯 생성
PATCH  /api/widgets/:id       - 위젯 수정
DELETE /api/widgets/:id       - 위젯 삭제
POST   /api/widgets/:id/copy  - 위젯 복사
GET    /api/widgets/:id/usage - 위젯 사용 정보 조회
POST   /api/widgets/:id/export - 위젯 데이터 내보내기 (Pro, US3.8)
```

## 3. Data Integration Feature Module
...
```
GET    /api/indicators              - 지표 검색
GET    /api/indicators/:id          - 지표 상세 조회
POST   /api/data/fetch              - 데이터 조회
POST   /api/data/batch              - 배치 데이터 조회
DELETE /api/data/cache/:indicatorId - 캐시 무효화
```
...
```
POST   /api/auth/signin/:provider - SNS 로그인 (google, kakao)
POST   /api/auth/signout          - 로그아웃
POST   /api/auth/refresh          - 세션 갱신
GET    /api/auth/user             - 현재 사용자 정보 조회
PATCH  /api/auth/profile          - 프로필 수정
POST   /api/auth/link/:provider   - SNS 계정 연결 (US1.2)
DELETE /api/auth/link/:provider   - SNS 계정 연결 해제 (US1.2)
```

## 5. Subscription Feature Module
...
GET    /api/subscription/limits   - 플랜 제한 조회 (동적)
GET    /api/subscription/usage    - 사용량 현황 조회
POST   /api/subscription/upgrade  - Pro 플랜 업그레이드
POST   /api/subscription/cancel   - 구독 취소
```

## 6. Admin Console Feature Module
...
**지표 관리 (US6.1, US6.2)**
```
GET    /api/admin/indicators           - 지표 목록 조회
POST   /api/admin/indicators           - 지표 등록
PATCH  /api/admin/indicators/:id       - 지표 수정
DELETE /api/admin/indicators/:id       - 지표 삭제
POST   /api/admin/indicators/:id/test  - 연결 테스트
```
...
PATCH  /api/admin/users/:id/subscription      - 구독 수동 변경
POST   /api/admin/users/:id/activate          - 계정 활성화
POST   /api/admin/users/:id/deactivate        - 계정 비활성화
GET    /api/admin/users/:id/activity          - 활동 로그 조회
```

**데이터 소스 관리 (US6.4)**
```
GET    /api/admin/datasources             - 데이터 소스 목록 조회
POST   /api/admin/datasources             - 데이터 소스 추가
PATCH  /api/admin/datasources/:id         - 데이터 소스 수정
PATCH  /api/admin/datasources/:id/apikey  - API Key 수정
PATCH  /api/admin/datasources/:id/status  - 활성화/비활성화
POST   /api/admin/datasources/:id/test    - 연결 테스트
```

**플랜 제한 관리 (US6.5)**
```
GET    /api/admin/plan-limits           - 플랜 제한 목록 조회
POST   /api/admin/plan-limits           - 플랜 제한 추가
PATCH  /api/admin/plan-limits/:id       - 플랜 제한 수정
DELETE /api/admin/plan-limits/:id       - 플랜 제한 삭제
GET    /api/admin/plan-limits/:key/history - 변경 이력 조회
```
...
PATCH  /api/admin/categories/:id/status - 활성화/비활성화
```

**수동 데이터 관리 (US6.8)**
```
POST   /api/admin/custom-data/:indicatorId/upload - CSV 업로드
POST   /api/admin/custom-data/:indicatorId        - 데이터 포인트 추가
GET    /api/admin/custom-data/:indicatorId        - 데이터 목록 조회
PATCH  /api/admin/custom-data/:id                 - 데이터 수정
DELETE /api/admin/custom-data/:id                 - 데이터 삭제
GET    /api/admin/custom-data/:indicatorId/history - 변경 이력 조회
```

**모니터링**
```
GET    /api/admin/metrics              - 시스템 메트릭 조회
GET    /api/admin/dashboard-stats      - 대시보드 통계 조회
```

## 7. Admin Moderation Feature Module

### 제공하는 인터페이스

```typescript
interface AdminModerationContract {
  // 신고 관리 (US6.9)
...
- `Authentication.getCurrentUser()` (관리자 인증)
- `Authentication.hasRole('admin')` (권한 확인)
- `Dashboard.getDashboards()` (신고된 대시보드 조회 - 읽기 전용)

### BFF API 엔드포인트 개요

**신고 관리 (US6.9)**
```
GET    /api/moderation/reports            - 신고된 대시보드 목록
POST   /api/moderation/reports/:id/review - 신고 검토 및 조치
```

**위반 사용자 관리 (US6.10)**
```
GET    /api/moderation/violations/users/:id - 사용자 위반 이력 조회
GET    /api/moderation/violations/stats     - 위반 통계 조회
POST   /api/moderation/users/:userId/suspend - 사용자 제재
```

**자동 탐지 시스템 (US6.11)**
```
GET    /api/moderation/auto-detect           - 자동 탐지된 컨텐츠 목록
GET    /api/moderation/detection-rules       - 탐지 룰 조회
POST   /api/moderation/detection-rules       - 탐지 룰 생성
PATCH  /api/moderation/detection-rules/:id   - 탐지 룰 수정
POST   /api/moderation/violations/:id/dismiss - 위반 무혐의 처리
```
...
- 각 Feature Module은 필요한 Feature Module의 인터페이스를 직접 import
...
각 Feature Module은 독립적으로 버전 관리되며, 인터페이스 변경 시:
...
각 Feature Module은 개발/테스트용 Mock 구현체 제공:

```typescript
// Mock 구현 예시
class MockAuthenticationModule implements AuthenticationContract {
  getCurrentUser() {
...
- 각 Feature Module은 독립적으로 배포 가능
- 인터페이스 변경 시 다른 Feature Module에 통지
